...
async def execute_okx_ticker(node: Dict, context: ExecutionContext) -> Any:
    """OKX行情节点 - 真实API调用"""
...
async def execute_okx_candles(node: Dict, context: ExecutionContext) -> Any:
    """OKX K线节点 - 真实API调用"""
    config = node.get("config", {})
    inst_id = config.get("inst_id", "BTC-USDT")
    bar = config.get(
    ...
    try:
        ...
        if success and data:
            ...
            context.variables[f"{inst_id}_closes"] = [c["close"] for c in candles]
...
async def execute_okx_orderbook(node: Dict, context: ExecutionContext) -> Any:
    """OKX订单簿节点 - 真实API调用"""
    config = node.get("config", {})
    inst_id = config.get("inst_id", "BTC-USDT")
...
async def execute_okx_account(node: Dict, context: ExecutionContext) -> Any:
    """OKX账户信息节点 - 真实API调用"""
...
async def execute_okx_positions(node: Dict, context: ExecutionContext) -> Any:
    """OKX持仓节点 - 真实API调用"""
    config = node.get("config", {})
